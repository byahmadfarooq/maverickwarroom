  ...
  return (
    <div>
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
        <Field label="Name" required><Input value={form.name} onChange={(e) => set('name', e.target.value)} placeholder="John Smith" /></Field>
        <Field label="Company" required><Input value={form.company} onChange={(e) => set('company', e.target.value)} placeholder="Acme Inc" /></Field>
      </div>
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
        <Field label="LinkedIn URL"><Input value={form.linkedinUrl} onChange={(e) => set('linkedinUrl', e.target.value)} placeholder="https://linkedin.com/in/..." /></Field>
        <Field label="Email"><Input value={form.email} onChange={(e) => set('email', e.target.value)} placeholder="john@example.com" type="email" /></Field>
      </div>
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 12 }}>
        ...
        <Field label="Deal Value"><Input value={form.dealValue} onChange={(e) => set('dealValue', e.target.value)} type="number" placeholder="3000" /></Field>
        <Field label="Status">
      ...
      <Field label="Next Follow-up"><Input value={form.nextFollowUp} onChange={(e) => set('nextFollowUp', e.target.value)} type="date" /></Field>
      ...
      <div style={{ display: 'flex', gap: 8, justifyContent: 'flex-end', marginTop: 16 }}>
        ...
        <Btn disabled={!valid} onClick={handleSubmit}>{prospect ? 'Update' : 'Add'} Prospect</Btn>
      </div>
    </div>
  );
};
